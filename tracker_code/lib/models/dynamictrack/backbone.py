...
from lib.models.dynamictrack import resnet as resnet_module
from lib.models.dynamictrack.repvgg import get_RepVGG_func_by_name
import os
...
def build_backbone(cfg):
    ...
    if cfg.MODEL.BACKBONE.TYPE1 == "RepVGG-A0":
        try:
        ...
        ckpt_path = None
    backbone1 = Backbone(cfg.MODEL.BACKBONE.TYPE1, train_backbone, return_interm_layers,
                        cfg.MODEL.BACKBONE.DILATION, cfg.TRAIN.FREEZE_BACKBONE_BN, ckpt_path)
    backbone2 = Backbone(cfg.MODEL.BACKBONE.TYPE2, train_backbone, return_interm_layers,
                         cfg.MODEL.BACKBONE.DILATION, cfg.TRAIN.FREEZE_BACKBONE_BN, ckpt_path)       # 20231028

    model1 = Joiner(backbone1, position_embedding)
    model2 = Joiner(backbone2, position_embedding)

    model1.num_channels = backbone1.num_channels
    model2.num_channels = backbone2.num_channels

    # model.num_channels1 = model1.num_channels
    # model.num_channels2 = model2.num_channels
...
def build_backbone_NOPE(cfg):
    ...
    if cfg.MODEL.BACKBONE.TYPE1 == "RepVGG-A0":
        ...
        ckpt_path = None
    backbone = Backbone(cfg.MODEL.BACKBONE.TYPE1, train_backbone, return_interm_layers,
                        cfg.MODEL.BACKBONE.DILATION, cfg.TRAIN.FREEZE_BACKBONE_BN, ckpt_path)
